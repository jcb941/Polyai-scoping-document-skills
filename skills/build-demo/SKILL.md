...
- **Never call `conv.say()` from `start_function.py` to deliver the voice greeting** — set `conv.state.first_utterance` and let `voice/configuration.yaml`'s `welcome_message: $first_utterance` speak it. `conv.say()` there looked fine in webchat/debug-chat testing but crashed real inbound SIP calls outright on a live Poly Bank build — the audio pipeline isn't necessarily ready for it at that point in a real call's setup. `$first_utterance` is the only pattern confirmed working end-to-end on a real phone call (Poly Hotel, `PROJECT-E8QG9DQ6`).
...
### Only add a language the platform actually, distinctly supports

Before adding any language beyond English to `SUPPORTED`/`rules.txt`/`phrase_filtering.yaml`, confirm it's a real, separately-selectable entry on the Behavior/Speech pages — not just a BCP-47 code that seems plausible. On a real Poly Bank build, adding `zh-HK` (Cantonese) — reasonable-looking, since the project already had `zh-CN` (Mandarin) — broke **100% of calls on the project**, voice and otherwise, with `config artifact is invalid: invalid language code for stop keyword: zh-HK` at call setup (confirmed via Datadog, every single error in the crash window was this one cause). The Behavior/Speech pages only ever expose one generic "Chinese," not Mandarin/Cantonese as distinct options — the platform has no stop-keyword (barge-in) resource for `zh-HK` at all, so any call setup that needs to build language artifacts for it fails outright, for every caller, not just ones who'd ask for Cantonese.

**The trap: this doesn't require enabling anything on the Behavior page to break.** A `language_code: zh-HK` entry in a single `phrase_filtering.yaml` rule was enough by itself to register the requirement and crash the whole project — there's no Behavior-page toggle to catch it, and `poly validate` doesn't flag it either (it's a runtime platform-config check, not a local one). If a crash report says "invalid language code for stop keyword: <code>," the fix is to remove every reference to that code project-wide (function `SUPPORTED` set, `rules.txt`, any `phrase_filtering.yaml` entry, hard-coded utterance dicts) — there's no way to "enable" a phantom language after the fact.
